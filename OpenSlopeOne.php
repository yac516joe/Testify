...
class OpenSlopeOne
{
    ...
    public function initSlopeOneTable($factory = 'PHP')
    {
        ...
        /**
         * Delete all the data of slope_one
         */
        $this->_db->query('TRUNCATE TABLE `slope_one`');
    ...
    {        
         ...
         */
        $sql = 'SELECT DISTINCT item_id FROM rating_db';
        $rs = $this->_db->query($sql);
        while ($itemId = $rs->fetchColumn())
        {
            $slopeOneSql = 'insert into slope_one (select a.item_id as item_id1,b.item_id as item_id2,count(*) as times, sum(a.rating-b.rating) as rating from rating_db a,rating_db b where a.item_id = '
                         . $itemId
        ...
        $sql = '
            CREATE PROCEDURE `slope_one`()
                begin                    
                    ...
                    DECLARE mycursor CURSOR FOR select distinct item_id from rating_db;
                    DECLARE CONTINUE HANDLER FOR NOT FOUND set done=1;
                    open mycursor;
                    while (!done) do
                        fetch mycursor into tmp_item_id;
                        if (!done) then
                            insert into slope_one (select a.item_id as item_id1,b.item_id as item_id2,count(*) as times, sum(a.rating-b.rating) as rating from rating_db a,rating_db b where a.item_id = tmp_item_id and b.item_id != a.item_id and a.user_id=b.user_id group by a.item_id,b.item_id);
                        end if;
                    END while;
                    close mycursor;
                end
